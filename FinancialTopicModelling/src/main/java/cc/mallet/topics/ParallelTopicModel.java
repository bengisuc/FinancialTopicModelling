...
//bengisu
	...
	public void estimate () throws IOException {
		...
		for (int iteration = 1; iteration <= numIterations; iteration++) {
			...
			/*if (showTopicsInterval != 0 && iteration != 0 && iteration % showTopicsInterval == 0) {
				logger.info(displayTopWords (wordsPerTopic, false)); 		//silindi -useful
			}*/
		...
		long seconds = Math.round(miliseconds /1000); miliseconds %= 1000; 	//bengisu
		...
		if (seconds != 0) { timeReport.append(seconds); timeReport.append(" seconds "); } 	//bengisu
		timeReport.append(miliseconds); timeReport.append(" miliseconds");
	...
	//bengisu buraya bak display/print
	...
	//topic[i] + weight + sorted word progress displayer(in console)
	...
	public void topicXMLReport (PrintWriter out, int numWords) {
		...
		for (int topic = 0; topic < numTopics; topic++) {
			...
			//while (iterator.hasNext() && rank <= numWords) //orjinal
			while (iterator.hasNext() && rank <= 5) { //bengisu - basılacak kelime sayısı ayarı
				IDSorter info = iterator.next();
	...
	public void printState (PrintStream out) {
		...
		out.println ("#doc source pos typeindex type topic"); //buraya bak
		out.print("#alpha : ");
		for (int topic = 0; topic < numTopics; topic++) {
